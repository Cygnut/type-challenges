...
interface Constructor<Constructed> {
  new(...args: any[]): Constructed
};

type InferConstructed<Type> = Type extends Constructor<infer Constructed> ? Constructed : never;

type TypeFromConstructor<T> = T extends String | Number | Boolean 
...
interface Props {
  [key: string]: { type?: Constructor<any> | [...Constructor<any>[]] } | Constructor<any>
};

// todo: map props
// todo: map computed

type PropsData<Props> = {
  [P in keyof Props]: Props[P] extends { type: infer Ctor } 
    ? TypeFromConstructor<InferConstructed<Ctor>>
    : TypeFromConstructor<InferConstructed<Props[P]>>
};

type Data<Props> = (this: PropsData<Props>) => { [key: string]: any };

interface Computed<Props, Data> {
  [key: string]: (this: PropsData<Props> & Data) => any;
};

interface Methods<Props, Data, Computed> {
  [key: string]: (this: PropsData<Props> & Data & Computed, ...args: any[]) => any;
};

declare function VueBasicProps<
  _Props extends Props, 
  _Data extends Data<_Props>, 
  _Computed extends Computed<_Props, _Data>,
  _Methods extends Methods<_Props, _Data, _Computed>
>(options: {
  props: _Props,
  data: _Data,
  computed: _Computed,
